...
class ServerFailure extends Failure {
  const ServerFailure({String message = 'Lỗi kết nối từ máy chủ'}) : super(message: message);
}

/// Lỗi xảy ra khi thiết bị mất kết nối Internet hoặc yêu cầu bị quá hạn (Timeout).
class NetworkFailure extends Failure {
  const NetworkFailure({String message = 'Không có kết nối Internet'}) : super(message: message);
}

/// Lỗi xảy ra khi không thể đọc hoặc ghi dữ liệu vào bộ nhớ cục bộ (Local Storage).
class CacheFailure extends Failure {
  const CacheFailure({String message = 'Lỗi truy xuất dữ liệu cục bộ'}) : super(message: message);
}
...
class UnknownFailure extends Failure {
  const UnknownFailure({String message = 'Đã xảy ra lỗi không xác định'}) : super(message: message);
}